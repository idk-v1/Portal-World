...
void Renderer::render(sf::RenderWindow& window, World& world, Player& player, int tileSize, std::vector<sf::Texture>& textures)
{
		...
		for (int l = 0; l < world.getLength(); l++)
		{
			...
			for (int h = 0; h < world.getHeight(); h++)
			{
				...
				if (draw && hgt == h)
				{
					...
					rect.setFillColor(sf::Color(96 + lvl * hgt, 96 + lvl * hgt, 96 + lvl * hgt));
